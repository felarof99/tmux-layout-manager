...
// SplitCurrentWindow splits the current window into panes based on the first
// window definition in the layout. Does not create new windows.
func SplitCurrentWindow(winTarget, startDir string, layout *config.LayoutConfig) error {
	...
	if len(win.Panes) <= 1 {
		if len(win.Panes) == 1 && win.Panes[0].Cmd != "" {
			target := "=" + winTarget + ".0"
			run("send-keys", "-t", target, "-l", win.Panes[0].Cmd)
	...
	if win.Rows > 1 {
		return splitCurrentWindowGrid(winTarget, startDir, win)
	}
	...
	for i := 1; i < len(win.Panes); i++ {
		...
		if _, err := run("split-window", splitFlag, "-t", "="+winTarget, "-p", strconv.Itoa(p), "-c", startDir); err != nil {
	...
	for i, pane := range win.Panes {
		if pane.Cmd != "" {
			target := fmt.Sprintf("=%s.%d", winTarget, i)
			run("send-keys", "-t", target, "-l", pane.Cmd)
			run("send-keys", "-t", target, "Enter")
		}
	}

	run("select-pane", "-t", "="+winTarget+".0")
	return nil
}

func splitCurrentWindowGrid(winTarget, startDir string, win config.WindowConfig) error {
	rows := win.Rows
	cols := (len(win.Panes) + rows - 1) / rows

	for c := 1; c < cols; c++ {
		remaining := cols - c
		total := remaining + 1
		p := remaining * 100 / total
		if _, err := run("split-window", "-h", "-t", "="+winTarget+".0", "-p", strconv.Itoa(p), "-c", startDir); err != nil {
			return fmt.Errorf("grid: creating column %d: %w", c, err)
		}
	}
